...
// Revision: A
//
// Description: This file contains the class definition for genericClientForce.
// genericClientForce is a generic element defined by any number of DOF.
// The element is a force-controlled element, meaning that it sends out
// forces at the interface DOF and requires motion feedback (i.e., disp, vel)
// at the interface DOF. The element communicates with an OpenFresco element
// through a tcp connection.

#define S_FUNCTION_NAME SFun_GenericClientForce
...
static void mdlStart(SimStruct *S)
{
    ...
    // get parameters
    int_T sizeTrialForce = ssGetInputPortWidth(S, 0);
    int_T sizeDaqDisp    = ssGetOutputPortWidth(S, 0);
    int_T sizeDaqVel     = ssGetOutputPortWidth(S, 1);
    int_T sizeDaqForce   = ssGetOutputPortWidth(S, 2);
    ...
    dataSize[0] = (1+sizeTrialForce+1>dataSize[0]) ? 1+sizeTrialForce+1 : dataSize[0];
    dataSize[0] = (sizeDaqDisp+sizeDaqVel+sizeDaqForce+1>dataSize[0]) ? sizeDaqDisp+sizeDaqVel+sizeDaqForce+1 : dataSize[0];
    ...
    iData[1] = 0;               // vel
    iData[2] = 0;               // accel
    iData[3] = sizeTrialForce;  // force
    iData[4] = 1;               // time
    // sizeDaq
    iData[5] = sizeDaqDisp;     // disp
    iData[6] = sizeDaqVel;      // vel
    iData[7] = 0;               // accel
...
{
    InputRealPtrsType trialForce  = ssGetInputPortRealSignalPtrs(S, 0);
    real_T *daqDisp  = ssGetOutputPortRealSignal(S, 0);
    real_T* daqVel   = ssGetOutputPortRealSignal(S, 1);
    real_T *daqForce = ssGetOutputPortRealSignal(S, 2);
    real_T* daqTime  = ssGetOutputPortRealSignal(S, 3);
    ...
    // send trial response
    sData[0] = 3;
    for (i=0; i<sizeTrialForce; i++)  {
        sData[1+i] = (*trialForce[i]);
    }
    sData[1+sizeTrialForce] = time[0];
    gMsg = (char_T *)sData;
    dataTypeSize = sizeof(real_T);
    nleft = dataSize[0];
    tcp_senddata(socketID, &dataTypeSize, gMsg, &nleft, &ierr);
    ...
    for (i=0; i<sizeDaqVel; i++) {
        daqDisp[i] = rData[sizeDaqDisp+i];
    }
    for (i=0; i<sizeDaqForce; i++) {
        daqForce[i] = rData[sizeDaqDisp+sizeDaqVel+i];
    }
    daqTime[i] = rData[sizeDaqDisp+sizeDaqVel+sizeDaqForce];
}
