...
// Revision: A
//
// Description: This file contains the class definition for genericClientDisp.
// genericClientDisp is a generic element defined by any number of DOF.
// The element is a displacement-controlled element, meaning that it sends
// out displacements, (velocities, an accelerations) at the interface DOF
// and requires force feedback at the interface DOF. The element communicates
// with an OpenFresco element through a tcp connection.

#define S_FUNCTION_NAME SFun_GenericClientDisp
...
static void mdlStart(SimStruct *S)
{
    ...
    int_T sizeTrialVel   = ssGetInputPortWidth(S, 1);
    int_T sizeTrialAccel = ssGetInputPortWidth(S, 2);
    ...
    dataSize[0] = (1+sizeTrialDisp+sizeTrialVel+sizeTrialAccel+1>dataSize[0]) ? 1+sizeTrialDisp+sizeTrialVel+sizeTrialAccel+1 : dataSize[0];
    dataSize[0] = (sizeDaqDisp+sizeDaqForce+1>dataSize[0]) ? sizeDaqDisp+sizeDaqForce+1 : dataSize[0];
    ...
    iData[1] = sizeTrialVel;    // vel
    iData[2] = sizeTrialAccel;  // accel
    iData[3] = 0;               // force
    iData[4] = 1;               // time
    // sizeDaq
...
{
    InputRealPtrsType trialDisp  = ssGetInputPortRealSignalPtrs(S, 0);
    InputRealPtrsType trialVel   = ssGetInputPortRealSignalPtrs(S, 1);
    InputRealPtrsType trialAccel = ssGetInputPortRealSignalPtrs(S, 2);
    real_T *daqDisp  = ssGetOutputPortRealSignal(S, 0);
    real_T *daqForce = ssGetOutputPortRealSignal(S, 1);
    real_T* daqTime  = ssGetOutputPortRealSignal(S, 2);
    ...
    for (i=0; i<sizeTrialDisp; i++)  {
        sData[1+i] = (*trialDisp[i]);
    }
    for (i=0; i<sizeTrialVel; i++) {
        sData[1+sizeTrialDisp+i] = (*trialVel[i]);
    }
    for (i = 0; i < sizeTrialDisp; i++) {
        sData[1+sizeTrialDisp+sizeTrialVel+i] = (*trialAccel[i]);
    }
    sData[1+sizeTrialDisp+sizeTrialVel+sizeTrialAccel] = time[0];
    gMsg = (char_T *)sData;
    dataTypeSize = sizeof(real_T);
    nleft = dataSize[0];
    tcp_senddata(socketID, &dataTypeSize, gMsg, &nleft, &ierr);
    ...
    daqTime[i] = rData[sizeDaqDisp+sizeDaqForce];
}
