...
// Revision: A
//
// Description: This file contains the implementation of the elementAPI class.
...
static void OPS_InvokeMaterialObject(struct matObject *theMat, modelState *theModel,
    double *strain, double *tang, double *stress, int *isw, int *result)
{
...
extern "C" int OPS_GetString(char *arrayData, int sizeArray)
{
    ...
    if (strlen(currentArgv[currentArg]) > (unsigned int)sizeArray) {
