...
int ESThreeActuatorsJntOff2d::transfDaqForce(Vector* force)
{
    ...
    // nonlinear geometry, horizontal actuator left
        ...
        double disp2 = atan((d2*cos(theta(2))+La1+L4-L5-La2-d1*cos(theta(1)))/(d2*sin(theta(2))+L1+L2-d1*sin(theta(1)))) - atan((L4-L5)/(L1+L2));
        double R0 = sqrt(L1*L1 + L4*L4);
        double R1 = sqrt(L2*L2 + L5*L5);
        double beta0 = atan(L4/L1) + disp2;
        double beta1 = atan(L5/L2) - disp2;
