...
void* OPF_ECSimSimulink()
{
    ...
    if (OPS_GetNumRemainingInputArgs() < 3) {
        ...
        opserr << "Want: expControl SimSimulink tag ipAddr ipPort <-udp> "
    ...
    theControl = new ECSimSimulink(tag, numTrialCPs, trialCPs,
...
ECSimSimulink::ECSimSimulink(int tag, int nTrialCPs, ExperimentalCP** trialcps,
    ...
    gotRelativeTrial(0)
{
    ...
    opserr << "* then start the Simulink model                                *\n";
    opserr << "*                                                              *\n";
    ...
    if (udp)
        theChannel = new UDP_Socket(ipPort, ipAddress, true);
    else
    ...
    opserr << "****************************************************************\n";
    opserr << "* The channel to Simulink with address: " << ipAddress << endln;
...
{
    // send termination to Simulink model (twice for reliability)
    ...
    opserr << "************************************************\n";
    opserr << "* The connection with Simulink has been closed *\n";
    opserr << "************************************************\n";
    opserr << endln;
}
...
int ECSimSimulink::setup()
{
    ...
    daqSignal = new Vector(&rData[1], numDaqSignals);
    daqSignal->Zero();
    ...
    // send the data sizes to the Simulink model
...
int ECSimSimulink::setTrialResponse(
    const Vector* disp,
    const Vector* vel,
    const Vector* accel,
    const Vector* force,
...
int ECSimSimulink::getDaqResponse(
    Vector* disp,
    Vector* vel,
    Vector* accel,
    Vector* force,
