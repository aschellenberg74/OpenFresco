...
// Revision: A
//
// Description: This file contains the class definition for OPF_UDPServer.
// OPF_UDPServer communicates with a SimSimulink experimental control in
// OpenFresco through a nonblocking udp connection.

#define S_FUNCTION_NAME SFun_OPF_UDPServer
...
void udp_sendnbdata(const int *socketID, int *dataTypeSize, char data[], int *lenData, int *ierr);
void udp_recvdata(const int *socketID, int *dataTypeSize, char data[], int *lenData, int *ierr);
void udp_recvnbdata(const int *socketID, int *dataTypeSize, char data[], int *lenData, int *ierr);
...
#define initHandshake(S)  ssGetSFcnParam(S,3)    // initial handshake flag
#define nbMode(S)         ssGetSFcnParam(S,4)    // nonblocking mode flag
...
static void mdlStart(SimStruct *S)
{
    ...
    int initHandshake = (int)mxGetScalar(initHandshake(S));
    udp_setupconnectionserver(&ipPort, initHandshake, socketID);
    if (socketID[0] < 0)  {
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    //    // stop the simulation
    //    ssSetStopRequested(S, 1);
    //    ssSetErrorStatus(S, "Failed to receive data from ECSimSimulink while executing udp_recvnbdata()");
    //}
}
