function Response = Integrator_AlphaOS(Model,GroundMotion,InitialState,Analysis)
%INTEGRATOR_ALPHAOS to perform a direct integration analysis using
% the second order Alpha-Operator-Splitting method
% Response = Integrator_AphaOS(Model,GroundMotion,InitialState,Analysis)
%
...
if ~isempty(InitialState)
    ...
    if isfield(InitialState,'Up') && ~isempty(InitialState.Up)
        Up(:,1:iStart) = InitialState.Up;
    end
    ...
    if isfield(InitialState,'P') && ~isempty(InitialState.P)
        P(:,1:iStart) = InitialState.P;
    end
