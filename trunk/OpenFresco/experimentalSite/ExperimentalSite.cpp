    ...
    daqFlag(false)
{
    ...
    if(sizeTrial == 0 || sizeOut == 0) {
        opserr << "FATAL ExperimentalSite::ExperimentalSite - "
            << "fail to create ID."
            << endln;
...
int ExperimentalSite::setDaqResponse(Vector* disp,
    Vector* vel,
    Vector* accel,
    Vector* force,
    Vector* time)
...
void ExperimentalSite::setTrial()
{
    ...
    if(size != 0) {
        ...
        if(tDisp == 0) {
            opserr << "FATAL ExperimentalSite::setTrial - "
    ...
    if(size != 0) {
        ...
        if(tTime == 0) {
            opserr << "FATAL ExperimentalSite::setTrialCPs - "
                << "failed to create Vector\n";
            exit(OF_ReturnType_failed);
        }
    }
...
void ExperimentalSite::setOut()
{
    ...
    if(size != 0) {
        ...
        if(Disp == 0) {
            opserr << "FATAL ExperimentalSite::setOut - "
                << "failed to create Vector\n";
            exit(OF_ReturnType_failed);
        }
    }
...
void ExperimentalSite::setTrialCPs(ArrayOfTaggedObjects &theCPs)
{
    ...
    if(cpsTrial != 0)
        delete cpsTrial;
    if(nTrial == 0) {
        opserr << "invalid size of cpsTrial";
    }
...
void ExperimentalSite::setOutCPs(ArrayOfTaggedObjects &theCPs)
{
    ...
    for(int i=1; i<=nOutput; i++) {
        TaggedObject *mc = theCPs.getComponentPtr(i);
        if(mc == 0) {
            opserr << "FATAL ExperimentalSite::setOutCPs - "
                << "failed to get ExperimentalCP\n";
            exit(OF_ReturnType_failed);
        }
        ExperimentalCP *theCP = (ExperimentalCP*)mc;
